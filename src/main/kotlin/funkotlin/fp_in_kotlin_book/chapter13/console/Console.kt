...
object ReadLine : Console<Option<String>>() {
    ...
    override fun toState(): ConsoleState<Option<String>> =
        ConsoleState({ b: Buffers ->
            val poppedElement: String? = b.input.lastOrNull() // 3
            val newList: List<String> = if (poppedElement != null) b.input.dropLast(1) else b.input
            Pair(poppedElement?.let { Some(it) } ?: None, b.copy(input = newList))
        })
...
data class PrintLine(val line: String) : Console<Unit>() {
    ...
    override fun toState(): ConsoleState<Unit> =
        ConsoleState({ b: Buffers -> Pair(Unit, b.copy(output = b.output + listOf(line)))})
}
...
fun consoleToFunction0() = object : Translate<ForConsole, ForFunction0> {
    override fun <A> invoke(fa: Kind<ForConsole, A>): Kind<ForFunction0, A> =
        Function0(fa.fix().toThunk())
}
fun consoleToPar() = object : Translate<ForConsole, ForPar> {
    override fun <A> invoke(fa: Kind<ForConsole, A>): Kind<ForPar, A> =
        fa.fix().toPar()
}
fun consoleToConsoleReader() = object : Translate<ForConsole, ForConsoleReader> {
    override fun <A> invoke(fa: Kind<ForConsole, A>): Kind<ForConsoleReader, A> =
        fa.fix().toReader()
}
fun consoleToConsoleState() = object : Translate<ForConsole, ForConsoleState> {
    override fun <A> invoke(fa: Kind<ForConsole, A>): Kind<ForConsoleState, A> =
        fa.fix().toState()
}
...
fun <A> runConsoleReader(a: ConsoleIO<A>): ConsoleReader<A> =
    runFree(a, consoleToConsoleReader(), ConsoleReader.monad()).fix().fix()

fun <A> runConsoleState(a: ConsoleIO<A>): ConsoleState<A> =
    runFree(a, consoleToConsoleState(), ConsoleState.monad()).fix().fix()
