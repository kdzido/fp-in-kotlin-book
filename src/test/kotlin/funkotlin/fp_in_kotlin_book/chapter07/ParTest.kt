...
class ParTest : FunSpec({
 ...
 test("should create unit of Par") {
  Pars.unit(2).run(pool).get() shouldBe 2
 }

 test("should sum2 ints") {
  sum2(listOf()).run(pool).get() shouldBe 0
  sum2(listOf(1)).run(pool).get() shouldBe 1
  sum2(listOf(1, 2, 3)).run(pool).get() shouldBe 6
 }

 test("should sum3 ints") { // hangs if not enough threads in pool
  sum3(listOf()).run(pool).get() shouldBe 0
  sum3(listOf(1)).run(pool).get() shouldBe 1
  sum3(listOf(1, 2, 3)).run(pool).get() shouldBe 6
 }
 ...
 test("should map2") {
  ...
  val mp = Pars.map2(p1, p2) { a, b -> a + b }.run(pool)
  mp.get() shouldBe 3
 }
 ...
 test("should sum with map3") {
  ...
  val mp = Pars.map3(p1, p2, p3) { a, b, c -> a + b + c }.run(pool)
  mp.get() shouldBe 6
 }
 ...
 test("should sum with map4") {
  ...
  val mp = Pars.map4(p1, p2, p3, p4) { a, b, c, d -> a + b + c + d}.run(pool)
  mp.get() shouldBe 10
 }
 ...
 test("should sum with map5") {
  ...
  val mp = Pars.map5(p1, p2, p3, p4, p5) { a, b, c, d, e -> a + b + c + d + e }.run(pool)
  mp.get() shouldBe 15
 }
 ...
 test("should asyncF") {
  val ap: Future<Int> = Pars.asyncF { a: Int -> Thread.sleep(50); a + 1 }(5).run(pool)
  ap.get() shouldBe 6
 }

 test("should sortPar") {
  val ap = Pars.sortPar(Pars.unit(listOf(3, 1, 4, 2)))
  ap.run(pool).get() shouldBe listOf(1, 2, 3, 4)
 }
